...
from model.HALO_Net import HALO
...
# from dataset.cal_sim import calc_sim, calc_sim_adapt
from dataset.cal_sim_sem_cluster import calc_sim
import warnings
...
def main():
    ...
    parser.add_argument('--max_epoch', default=40, type=int, help='training epoch')
    parser.add_argument('--lr', default=1e-4, type=float, help='learning rate')#1e-4,8e-5
    ...
    parser.add_argument('--data_root', type=str, default="/home/zzh/CVOGL/", help='path to the root folder of all dataset')
    ...
    parser.add_argument('--seed', default=12, type=int, help='random seed')#13
    ...
    parser.add_argument('--neighbour_range', default=64, type=int, help='neighbour_range')
    parser.add_argument('--normalize_features', default=True, type=bool, help='normalize_features')
    parser.add_argument('--margin', default=0.3, type=float, help='margin value')
    ...
    test_dataset = RSDataset(data_root=args.data_root,
                         ...
                         img_size = args.img_size,
                         transform=input_transform)
    sample_dataset = RSDataset(data_root=args.data_root,
                         data_name=args.data_name,
                         split_name='train',
                         img_size=args.img_size,
                         transform=input_transform)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True,
                              pin_memory=True, drop_last=False, num_workers=args.num_workers)#是否舍弃最后的batch
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False,
                              pin_memory=True, drop_last=False, num_workers=args.num_workers)
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False,
                              pin_memory=True, drop_last=False, num_workers=args.num_workers)
    sample_loader = DataLoader(sample_dataset,batch_size=args.batch_size, shuffle=False,
                              pin_memory=True, drop_last=False, num_workers=args.num_workers)
    
    # with open('/home/zzh/Desktop/CVOGL/CVOGL_DroneAerial/similarity_DroneAerial_rsimg_SSIM_train.pkl', "rb") as f:
    #     sim_dict = pickle.load(f)

    # train_loader.dataset.shuffle(sim_dict=sim_dict)
    sim_dict = None
    ...
    if args.test:
        ...
        for epoch in range(args.max_epoch):
            ...
            if (epoch!=0 and epoch %5 == 0):
                sim_dict = calc_sim(args, model, sample_loader)
                # sim_dict = calc_sim_adapt(args, model, sample_loader, epoch, args.max_epoch)
                print('更新sim_dict')
...
def train_epoch(train_loader, model, optimizer, epoch, args):
    ...
    avg_sample_losses = AverageMeter()
    avg_triple_losses = AverageMeter()
    ...
    for batch_idx, (query_imgs, rs_imgs, mat_clickxy, ori_gt_bbox, _) in enumerate(train_loader): #zzh
        ...
        pred_anchor, _, sample4_loss = model(query_imgs, rs_imgs, mat_clickxy)
        # print('qry_cluster_feat_type', type(qry_cluster_feat))
        # print('qry_cluster_feat', len(qry_cluster_feat))#3
        # print(qry_cluster_feat[0].size())# 16*512
        ...
        avg_sample_losses.update(sample4_loss.mean().item(), query_imgs.shape[0])
        
        
        accu_list, accu_center, iou, _, _, _, _ = eval_iou_acc(pred_anchor, ori_gt_bbox, anchors_full, best_anchor_gi_gj[:, 1], best_anchor_gi_gj[:, 2], args.img_size, iou_threshold_list=[0.5])
        accu = accu_list[0]
        ## metrics
        ...
        if batch_idx % args.print_freq == 0:
            print_str = 'Epoch: [{0}][{1}/{2}]\t' \
                ...
                'sample Loss {sample.val:.4f} ({sample.avg:.4f})\t' \
                'Accu {accu.val:.4f} ({accu.avg:.4f})\t' \
                'Mean_iou {miou.val:.4f} ({miou.avg:.4f})\t' \
                'Accu_c {accu_c.val:.4f} ({accu_c.avg:.4f})\t' \
                .format( \
                    epoch, batch_idx, len(train_loader), batch_time=batch_time, \
                    loss=avg_losses, geo=avg_geo_losses, cls=avg_cls_losses, sample=avg_sample_losses, accu=avg_accu, miou=avg_iou, accu_c=avg_accu_center)
            print(print_str)
...
def test_epoch(data_loader, model, args):
    ...
    # for batch_idx, (query_imgs, rs_imgs, mat_clickxy, ori_gt_bbox, _, queryimg_name, rsimg_name) in enumerate(data_loader):
        query_imgs, rs_imgs = query_imgs.cuda(), rs_imgs.cuda()
        mat_clickxy = mat_clickxy.cuda()
        ori_gt_bbox = ori_gt_bbox.cuda()
        ori_gt_bbox = torch.clamp(ori_gt_bbox, min=0, max=args.img_size-1)
        ...
        accu_list, accu_center, iou, each_acc_list, _, _, _= eval_iou_acc(pred_anchor, ori_gt_bbox, anchors_full, best_anchor_gi_gj[:, 1], best_anchor_gi_gj[:, 2], args.img_size, iou_threshold_list=[0.5, 0.25])
        # accu_list, accu_center, iou, each_acc_list, _, _ = eval_iou_acc(pred_anchor, ori_gt_bbox, anchors_full, best_anchor_gi_gj[:, 1], best_anchor_gi_gj[:, 2], args.img_size, iou_threshold_list=[0.5, 0.25],queryimg_name=queryimg_name, rsimg_name=rsimg_name)
